...
void RegisterProfilerSymbols(const char* symbols_path, const char* dso_name) {
  ...
  if (fx::ReadFileToString(symbols_path, symbols)) {
    ...
    FX_LOGF(ERROR, FX_LOG_TAG, "Failed to load %s.", symbols_path);
  }
}
#endif  // !defined(DART_PRODUCT)
...
bool InitializeTZData() {
  ...
  if (fd < 0) {
    FX_LOGF(ERROR, FX_LOG_TAG,
            "Could not open '%s', error: %s; proceeding without loading the "
            "timezone database.",
            tzdata_dir.c_str(), strerror(errno));
    return false;
  }
  if (!close(fd)) {
    FX_LOGF(WARNING, FX_LOG_TAG, "Could not close '%s', error: %s.",
            tzdata_dir.c_str(), strerror(errno));
  }
  return true;
}
...
Runner::Runner(Renderer::FactoryCallback renderer_factory, async::Loop* loop)
  ...
  //       FX_LOGF(WARNING, FX_LOG_TAG, "Failed to connect to %s: %s This is not
  //       a fatal error, but the user locale preferences will not be forwarded
  //       to Flutter apps.", fuchsia::intl::PropertyProvider::Name_,
  ...
  //         FX_LOG(WARNING, FX_LOG_TAG, "Got intl Profile without locales");
  //       }

  //       FX_VLOG(-1, FX_LOG_TAG, "Sending LocalizationPlatformMessage");
  ...
  //           FX_LOGF(ERROR, FX_LOG_TAG, "Error parsing locale ID \"%s\"",
  //           locale_id.id); continue;
  //         }
  ...
  //     FX_VLOG(-1, FX_LOG_TAG, "Requesting intl Profile");
  ...
  //           FX_VLOG(-1, FX_LOG_TAG, "%s: OnChange",
...
#if !NDEBUG
  FX_LOG(INFO, FX_LOG_TAG, "Flutter runner services initialized.");
#endif
}

Runner::~Runner() {
#if !NDEBUG
  FX_LOG(INFO, FX_LOG_TAG, "Flutter runner services stopped.");
#endif
