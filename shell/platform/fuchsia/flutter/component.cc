...
Component::Component(Context component_context, ThreadHost threads)
    : threads_(std::move(threads)),
      ...
      renderer_(component_context.renderer_factory_callback(Renderer::Context{
          ...
          .render_dispatcher = threads_.gpu_thread.dispatcher(),
  ...
  flutter_directory_.set_error_handler([this](zx_status_t status) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Interface error %s on %s",
            zx_status_get_string(status), "fuchsia::io::Directory");
    Kill();
  });
  controller_binding_.set_error_handler([this](zx_status_t status) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Interface error (binding) %s on %s",
            zx_status_get_string(status), "fuchsia::sys::ComponentController");
    Kill();
  });
...
void Component::SetupOutgoingDirectory() {
  ...
  flutter_directory_.events()
      .OnOpen = [this](zx_status_t status,
                       std::unique_ptr<fuchsia::io::NodeInfo> info) {
    // component_directory.events().OnOpen = nullptr;  // TODO(dworsham)
    if (status != ZX_OK) {
      FX_LOGF(ERROR, FX_LOG_TAG,
              "Could not bind out directory for flutter app (%s): %s",
              debug_label_.c_str(), zx_status_get_string(status));
      return;
    }
    ...
    for (auto& dir_str : other_dirs) {
      ...
      auto status = fdio_service_connect_at(flutter_directory_.channel().get(),
      ...
      if (status == ZX_OK) {
        ...
        FX_LOGF(
            ERROR, FX_LOG_TAG,
            "Could not add out directory entry(%s) for flutter app (%s): %s",
            dir_str, debug_label_.c_str(), zx_status_get_string(status));
      }
    }
  };
...
void Component::ParseStartupInfo(fuchsia::sys::StartupInfo startup_info) {
  ...
  if (assets_path_.empty()) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Could not find a package data directory for %s",
            component_url_.c_str());
    return;
  }
...
#if !NDEBUG
      FX_LOGF(ERROR, FX_LOG_TAG, "Could not bind path to namespace: %s",
              path.c_str());
#endif
  ...
  // Open the assets directory in order to load AOT or JIT snapshots.
  {
    ...
    if (component_directory != -1) {
      ...
      FX_LOG(ERROR, FX_LOG_TAG, "Failed to open root component directory.");
      FX_DCHECK(false);
    }
  ...
  if (FlutterEngineRunsAOTCompiledDartCode() &&
    ...
    if (vm_data == nullptr || vm_instructions == nullptr ||
      ...
      FX_LOG(FATAL, FX_LOG_TAG, "ELF snapshot missing AOT symbols.");
    }
    ...
    if (fx::ReadFileToStringAt(assets_directory_, "runner.frameworkversion",
                               &runner_framework) &&
        fx::ReadFileToStringAt(assets_directory_, "app.frameworkversion",
                               &app_framework) &&
        (runner_framework.compare(app_framework) == 0)) {
      FX_LOGF(INFO, FX_LOG_TAG, "Using snapshot with framework for %s",
              component_url_.c_str());

      framework_prefix = "framework_";
    } else {
      FX_LOGF(INFO, FX_LOG_TAG, "Using snapshot without framework for %s",
              component_url_.c_str());
    }
    vm_snapshot_data_mapping_ = fx::MappedResource::MakeFileMapping(
        assets_directory_, framework_prefix + "vm_snapshot_data.bin", false);
    isolate_snapshot_data_mapping_ = fx::MappedResource::MakeFileMapping(
        assets_directory_,
        framework_prefix + "framework_isolate_core_snapshot_data.bin", false);
    vm_snapshot_instructions_mapping_ = fx::MappedResource::MakeFileMapping(
        assets_directory_, "vm_snapshot_instructions.bin", true);
    isolate_snapshot_instructions_mapping_ =
        fx::MappedResource::MakeFileMapping(
            assets_directory_, "isolate_core_snapshot_instructions.bin", true);
  }
...
void Component::LaunchFlutter() {
  ...
  FlutterCompositor embedder_compositor_callbacks = {
      ...
      .create_backing_store_callback =
          [](const FlutterBackingStoreConfig* layer_config,
            ...
            bool status =
                renderer->CreateBackingStore(layer_config, backing_store_out);
            if (!status) {
              FX_LOG(ERROR, FX_LOG_TAG, "Failed creating backing store");
              return false;
            }
      ...
      .collect_backing_store_callback =
          [](const FlutterBackingStore* backing_store, void* user_data) {
            ...
            bool status = renderer->CollectBackingStore(backing_store);
            if (!status) {
              FX_LOG(ERROR, FX_LOG_TAG, "Failed collecting backing store");
              return false;
            }
      ...
      .present_layers_callback =
          [](const FlutterLayer** layers, size_t layer_count, void* user_data) {
            ...
            bool status = renderer->PresentLayers(layers, layer_count);
            if (!status) {
              FX_LOG(ERROR, FX_LOG_TAG, "Failed presenting layers");
              return false;
            }
...
#if !NDEBUG
    FX_LOGF(INFO, FX_LOG_TAG, "Main isolate for engine '%s' was started.",
            component->debug_label_.c_str());
#endif  // !NDEBUG
    fidl::InterfaceHandle<fuchsia::sys::Environment> environment;
...
#if !NDEBUG
    FX_LOGF(INFO, FX_LOG_TAG, "Main isolate for engine '%s' shutting down.",
            component->debug_label_.c_str());
#endif  // !NDEBUG
  ...
  embedder_args.vsync_callback = [](void* user_data, intptr_t baton) {
    FX_LOG(ERROR, FX_LOG_TAG, "EMBEDDER VSYNC CALLBACK");
    Component* component = static_cast<Component*>(user_data);
    FX_DCHECK(component);

    component->renderer_->AwaitPresent(baton);
  };
  ...
  FX_LOG(ERROR, FX_LOG_TAG, "STARTING ENGINE");
  ...
  // runner_incoming_services_->Connect(sync_font_provider.NewRequest());
