...
ScenicInputHandler::ScenicInputHandler(
  ...
  ime_client_.set_error_handler([error_callback](zx_status_t status) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Interface error (binding) %s on %s",
            zx_status_get_string(status),
            "fuchsia::ui::input::InputMethodEditorClient");
    error_callback();
  });
  ime_.set_error_handler([error_callback](zx_status_t status) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Interface error %s on %s",
            zx_status_get_string(status),
            "fuchsia::ui::input::InputMethodEditor");
    error_callback();
  });
  text_sync_service_.set_error_handler([error_callback](zx_status_t status) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Interface error %s on %s",
            zx_status_get_string(status), "fuchsia::ui::input::ImeService");
    error_callback();
  });
...
void ScenicInputHandler::OnScenicEvent(
  ...
  TRACE_DURATION("flutter", "ScenicInputHandler::OnScenicEvent");
      ...
      case fuchsia::ui::scenic::Event::Tag::kGfx:
        switch (event.gfx().Which()) {
          ...
          case fuchsia::ui::gfx::Event::Tag::Invalid:
            FX_LOG(ERROR, FX_LOG_TAG,
                   "Flutter ScenicInputHandler::OnScenicEvent: Got an invalid "
                   "GFX event.");
            FX_DCHECK(false);
            break;
          default:
...
void ScenicInputHandler::PlatformMessageResponse(
  ...
  if (found != platform_message_handlers_.end()) {
    ...
    FX_LOGF(ERROR, FX_LOG_TAG,
            "Platform received message on channel '%s' with no registered "
            "handler. And empty response will be generated. Please implement "
...
void ScenicInputHandler::DidUpdateState(
  ...
  auto result = FlutterEngineSendPlatformMessage(nullptr, &message);
  FX_DCHECK(result == kSuccess);
...
void ScenicInputHandler::OnPropertiesChanged(
  ...
  auto result = FlutterEngineSendWindowMetricsEvent(nullptr, &current_metrics_);
  FX_DCHECK(result == kSuccess);
}
...
bool ScenicInputHandler::OnHandlePointerEvent(
  ...
  TRACE_DURATION("flutter", "ScenicInputHandler::OnHandlePointerEvent");
  ...
  switch (pointer_event.phase) {
    ...
    case FlutterPointerPhase::kAdd:
      if (down_pointers_.count(pointer_event.device) != 0) {
        FX_LOG(ERROR, FX_LOG_TAG, "Received add event for down pointer.");
      }
      break;
    case FlutterPointerPhase::kRemove:
      if (down_pointers_.count(pointer_event.device) != 0) {
        FX_LOG(ERROR, FX_LOG_TAG, "Received remove event for down pointer.");
      }
      break;
    case FlutterPointerPhase::kHover:
      if (down_pointers_.count(pointer_event.device) != 0) {
        FX_LOG(ERROR, FX_LOG_TAG, "Received hover event for down pointer.");
      }
      break;
  }

  auto result = FlutterEngineSendPointerEvent(nullptr, &pointer_event, 1);
  FX_DCHECK(result == kSuccess);
...
bool ScenicInputHandler::OnHandleKeyboardEvent(
  ...
  if (type == nullptr) {
    FX_LOG(ERROR, FX_LOG_TAG, "Unknown key event phase.");
    return false;
  }
...
bool ScenicInputHandler::HandleFlutterTextInputChannelPlatformMessage(
  ...
  if (method->value == "TextInput.show") {
    ...
    FX_LOGF(ERROR, FX_LOG_TAG, "Unknown %s method %s", message->channel,
...
bool ScenicInputHandler::HandleFlutterPlatformViewsChannelPlatformMessage(
  ...
  document.Parse(reinterpret_cast<const char*>(message->message),
                 message->message_size);
  if (document.HasParseError() || !document.IsObject()) {
    FX_LOG(ERROR, FX_LOG_TAG, "Could not parse document");
    return false;
  }
  auto root = document.GetObject();
  auto method = root.FindMember("method");
  ...
  if (method->value == "View.enableWireframe") {
    auto args_it = root.FindMember("args");
    if (args_it == root.MemberEnd() || !args_it->value.IsObject()) {
      FX_LOG(ERROR, FX_LOG_TAG, "No arguments found.");
      return false;
    }
    const auto& args = args_it->value;
    ...
    if (!enable->value.IsBool()) {
      FX_LOG(ERROR, FX_LOG_TAG, "Argument 'enable' is not a bool");
      return false;
    }
