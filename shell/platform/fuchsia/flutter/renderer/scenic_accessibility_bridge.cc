...
ScenicAccessibilityBridge::ScenicAccessibilityBridge(
  ...
  fuchsia_semantics_manager_.set_error_handler([](zx_status_t status) {
    FX_LOGF(ERROR, FX_LOG_TAG,
            "Flutter cannot connect to SemanticsManager with status: %s.",
...
void ScenicAccessibilityBridge::AddSemanticsNodeUpdate(
    const FlutterSemanticsNode* node) {
  FX_DCHECKM(
      nodes_.find(kRootNodeId) != nodes_.end() || node->id == kRootNodeId,
  ...
  if (this_node_size >= kMaxMessageSize) {
    FX_LOGF(ERROR, FX_LOG_TAG,
            "Semantics node with ID %i exceeded the maximum FIDL message size "
            "and may not be delivered to the accessibility manager service.",
            node->id);
    return;
  }
    ...
    if (it != nodes_.end()) {
      ...
      for (const auto& child : children) {
        if (descendents.find(child) == descendents.end()) {
          ...
          FX_LOGF(ERROR, FX_LOG_TAG,
                  "Semantics Node %i has already been listed as a child of "
                  "another node, ignoring for parent %i.",
                  child, id);
        }
      }
    }
