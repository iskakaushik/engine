...
void Initialize(
  ...
  FX_LOG(ERROR, FX_LOG_TAG, "7");
  auto env_handle2 = env_handle->shared_from_this();
  FX_LOG(ERROR, FX_LOG_TAG, "7.2");
  auto env_dart_handle = tonic::ToDart(std::move(env_handle));
  result =
      Dart_SetField(library, tonic::ToDart("_environment"), env_dart_handle);
  FX_CHECK(!tonic::LogIfError(result));
