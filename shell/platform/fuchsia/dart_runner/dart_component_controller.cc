...
#include "flutter/shell/platform/fuchsia/utils/handle_exception.h"
...
#include "flutter/third_party/tonic/dart_message_handler.h"
...
// Forward declaration needed for kLoopConfig.
...
DartComponentController::DartComponentController(
  ...
  if (data_path_.empty()) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Could not find a /pkg/data directory for %s",
            url_.c_str());
    return;
  }
  ...
  if (status != ZX_OK) {
    FX_LOGF(INFO, FX_LOG_TAG, "Idle timer creation failed: %s",
...
bool DartComponentController::Setup() {
  ...
  if (SetupFromAppSnapshot()) {
    FX_LOGF(INFO, FX_LOG_TAG, "%s is running from an app snapshot",
            url_.c_str());
  } else if (SetupFromKernel()) {
    FX_LOGF(INFO, FX_LOG_TAG, "%s is running from kernel", url_.c_str());
  } else {
...
bool DartComponentController::Main() {
  ...
  if (!svc) {
    FX_LOG(ERROR, FX_LOG_TAG, "Unable to get /svc for dart component");
    return false;
  }
  ...
  InitBuiltinLibraries(url_, namespace_, std::move(environment),
                       std::move(directory_request), stdoutfd_, stderrfd_,
  ...
  if (error != nullptr) {
    ...
    FX_LOGF(ERROR, FX_LOG_TAG, "Unable to make isolate runnable: %s", error);
    free(error);
    return false;
  }
  Dart_EnterIsolate(isolate_);
  ...
  if (Dart_IsError(dart_arguments)) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Failed to allocate Dart arguments list: %s",
  ...
  for (size_t i = 0; i < arguments.size(); i++) {
    tonic::LogIfError(
        Dart_ListSetAt(dart_arguments, i, tonic::ToDart(arguments.at(i))));
  }
  ...
  Dart_Handle main_result = Dart_Invoke(
      Dart_RootLibrary(), tonic::ToDart("main"), std::size(argv), argv);
  if (Dart_IsError(main_result)) {
    ...
    if (!dart_state->has_set_return_code()) {
      ...
      FX_LOG(ERROR, FX_LOG_TAG, Dart_GetError(main_result));
      return_code_ = tonic::GetErrorExitCode(main_result);

      fx::HandleIfException(runner_incoming_services_, url_, main_result);
    }
    Dart_ExitScope();
    return false;
  }
...
bool DartComponentController::SetupNamespace() {
  ...
  if (status != ZX_OK) {
    FX_LOG(ERROR, FX_LOG_TAG, "Failed to create namespace");
    return false;
  }

  fx::RunnerTemp::SetupComponent(namespace_);
  ...
  for (size_t i = 0; i < flat->paths.size(); ++i) {
    ...
    if (status != ZX_OK) {
      FX_LOGF(ERROR, FX_LOG_TAG, "Failed to bind %s to namespace: %s",
...
bool DartComponentController::SetupFromKernel() {
  ...
  for (size_t start = 0; start < manifest.size();) {
    ...
    if (end == std::string::npos) {
      FX_LOG(ERROR, FX_LOG_TAG, "Malformed manifest");
      Dart_ExitScope();
      return false;
    }
    ...
    fx::MappedResource kernel;
    if (!fx::MappedResource::LoadFromNamespace(namespace_, path, kernel)) {
      FX_LOGF(ERROR, FX_LOG_TAG, "Failed to find kernel: %s", path.c_str());
      Dart_ExitScope();
      return false;
    }
    library = Dart_LoadLibraryFromKernel(kernel.address(), kernel.size());
    if (Dart_IsError(library)) {
      FX_LOGF(ERROR, FX_LOG_TAG, "Failed to load kernel: %s",
  ...
  if (Dart_IsError(result)) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Failed to FinalizeLoading: %s",
...
int DartComponentController::SetupFileDescriptor(
  ...
  if (status != ZX_OK) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Failed to extract output fd: %s",
            zx_status_get_string(status));
    return -1;
  }
  return outfd;
...
bool DartComponentController::CreateIsolate(
  ...
  if (!isolate_) {
    FX_LOGF(ERROR, FX_LOG_TAG, "Dart_CreateIsolateGroup failed: %s", error);
    return false;
  }

  state->get()->SetIsolate(isolate_);

  tonic::DartMessageHandler::TaskDispatcher dispatcher = [this](auto callback) {
    async::PostTask(loop_.dispatcher(), std::move(callback));
  };
...
void DartComponentController::MessageEpilogue(Dart_Handle result) {
  ...
  fx::HandleIfException(runner_incoming_services_, url_, result);
  ...
  if (status != ZX_OK) {
    FX_LOGF(INFO, FX_LOG_TAG, "Idle timer set failed: %s",
            zx_status_get_string(status));
  }
}
