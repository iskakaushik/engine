...
DartRunner::DartRunner() : context_(sys::ComponentContext::Create()) {
  ...
  if (error) {
    FX_LOGF(FATAL, FX_LOG_TAG, "Dart_SetVMFlags failed: %s", error);
  }
  ...
  if (!fx::MappedResource::LoadFromNamespace(
    ...
    FX_LOG(FATAL, FX_LOG_TAG, "Failed to load vm snapshot data");
  }
  if (!fx::MappedResource::LoadFromNamespace(
          nullptr, "pkg/data/vm_snapshot_instructions.bin",
          vm_snapshot_instructions_, true /* executable */)) {
    FX_LOG(FATAL, FX_LOG_TAG, "Failed to load vm snapshot instructions");
  }
  ...
  if (error)
    FX_LOGF(FATAL, FX_LOG_TAG, "Dart_Initialize failed: %s", error);
}

DartRunner::~DartRunner() {
  char* error = Dart_Cleanup();
  if (error)
    FX_LOGF(FATAL, FX_LOG_TAG, "Dart_Cleanup failed: %s", error);
}
