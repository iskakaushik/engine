...
bool SetStackTrace(const std::string& data,
...
fuchsia::feedback::CrashReport BuildCrashReport(
  ...
  if (!SetStackTrace(stack_trace, &dart_report)) {
    FX_LOG(ERROR, FX_LOG_TAG, "Failed to convert Dart stack trace to VMO");
  }
...
std::string StringFromDart(Dart_Handle handle) {
...
void HandleIfException(std::shared_ptr<::sys::ServiceDirectory> services,
  ...
  const std::string error =
      StringFromDart(Dart_ToString(Dart_ErrorGetException(result)));
  const std::string stack_trace =
      StringFromDart(Dart_ToString(Dart_ErrorGetStackTrace(result)));
...
void HandleException(std::shared_ptr<::sys::ServiceDirectory> services,
  ...
  crash_reporter->File(
      ...
      [](fuchsia::feedback::CrashReporter_File_Result result) {
        if (result.is_err()) {
          FX_LOGF(ERROR, FX_LOG_TAG, "Failed to report Dart exception: %d (%s)",
