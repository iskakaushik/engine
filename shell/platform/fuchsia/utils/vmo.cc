...
bool VmoFromFd(int fd, fuchsia::mem::Buffer* buffer) {
  if (!buffer) {
    FX_LOG(FATAL, FX_LOG_TAG, "Invalid buffer pointer");
  }

  struct stat stat_struct;
  if (fstat(fd, &stat_struct) == -1) {
    FX_LOGF(ERROR, FX_LOG_TAG, "fstat failed: %s", strerror(errno));
    return false;
  }
...
bool VmoFromFilenameAt(int dirfd,
  ...
  if (fd == -1) {
    FX_LOGF(ERROR, FX_LOG_TAG, "openat(\"%s\") failed: %s", filename.c_str(),
            strerror(errno));
    return false;
  }
